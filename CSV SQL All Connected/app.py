...
from PyQt6.QtWidgets import (
    ...
    QAbstractItemView, QComboBox, QCompleter, QTextEdit, QMenu, 
    QDialog, QMessageBox, QLineEdit, QListWidget, QHeaderView
)
# from PyQt6.QtWidgets import QPlainTextEdit, QSizePolicy, QInputDialog, QApplication
...
from editor import SQLTextEdit
from db_manager import SQLConnections
...
class CSVSQLApp(QMainWindow):
    def __init__(self):
        ...
        self.db_manager = SQLConnections()
        self.pg_conn_file = os.path.join(os.path.dirname(os.path.abspath(__file__)), "pg_connections.json")
        self.auto_reconnect_last_pg()
        self.saved_pg_connections = {}
        self.tables = {}
        self.name_map = {}
        self.saved_queries = {}
        ...
        self.connect_pg_btn = QPushButton("CONNECT DB")
        self.clear_btn = QPushButton("CLEAR")
        self.help_btn = QPushButton("HELP")
        self.export_btn = QPushButton("EXPORT")
        self.queries_btn = QPushButton("CUSTOM QUERIES")
        self.buttons = [
            self.load_btn, self.cancel_btn, self.queries_btn,
            self.run_btn, self.connect_pg_btn,
            self.clear_btn, self.help_btn, self.export_btn
        ]
        self.result_info = QLabel("Rows affected: 0 | Columns: 0")
        self.db_status = QLabel()
        self.update_db_status("SQLite (Default)", "#f39c12")
        ...
        self.db_mode_selector = QComboBox()
        self.db_mode_selector.setToolTip("SQLite is active. PostgreSQL/MySQL require external connection setup.")
        self.db_mode_selector.addItems(sorted(self.keyword_data.keys()))
        self.db_mode_selector.currentIndexChanged.connect(self.update_db_mode_status)
        ...
        self.db_mode_selector.currentIndexChanged.connect(self.refresh_highlighter)
        ...
        self.pg_saved_dropdown = QComboBox()
        self.load_pg_connections()
        self.pg_saved_dropdown.addItem("Select Saved Connection")
        self.pg_saved_dropdown.currentIndexChanged.connect(self.load_saved_pg_connection)
        self.pg_host_input.setPlaceholderText("Host (e.g. localhost)")
        self.pg_name_input = QLineEdit()
        self.pg_name_input.setPlaceholderText("Connection Name (e.g. localhost_postgres)")
        self.pg_db_input = QLineEdit()
        self.pg_db_input.setPlaceholderText("Database")
        self.pg_host_input.textChanged.connect(self.auto_fill_conn_name)
        self.pg_user_input = QLineEdit()
        self.pg_user_input.setPlaceholderText("User")
        self.pg_pass_input = QLineEdit()
        self.pg_pass_input.setPlaceholderText("Password")
        self.pg_pass_input.setEchoMode(QLineEdit.EchoMode.Password)
        self.pg_connect_btn = QPushButton("CONNECT DB")
        self.pg_disconnect_btn = QPushButton("DISCONNECT DB")
        self.pg_save_btn = QPushButton("SAVE CONN")
        self.pg_delete_btn = QPushButton("DELETE CONN")
        self.pg_delete_btn.clicked.connect(self.delete_pg_connection)
        self.pg_save_btn.clicked.connect(self.save_pg_connection)
        top_bar = QHBoxLayout()
        db_bar = QHBoxLayout()
        db_bar.addWidget(self.pg_saved_dropdown)
        db_bar.addWidget(self.pg_host_input)
        db_bar.addWidget(self.pg_name_input)
        db_bar.addWidget(self.pg_db_input)
        db_bar.addWidget(self.pg_user_input)
        db_bar.addWidget(self.pg_pass_input)
        db_bar.addWidget(self.pg_connect_btn)
        db_bar.addWidget(self.pg_disconnect_btn)
        db_bar.addWidget(self.pg_save_btn)
        db_bar.addWidget(self.pg_delete_btn)
        ...
        status_bar.addWidget(self.db_mode_selector, 0)
        status_bar.addWidget(self.db_status, 0)
        status_bar.addWidget(self.source_info, 0)
        status_bar.addWidget(self.result_info, 0)
        status_bar.addStretch(1)
        top_widget = QWidget()
        top_layout = QVBoxLayout(top_widget)
        top_layout.addLayout(db_bar)
        ...
        self.pg_connect_btn.clicked.connect(self.connect_postgres_ui)
        self.pg_disconnect_btn.clicked.connect(self.disconnect_db)
    ...
    def update_db_status(self, text, color="#f39c12"):
        self.db_status.setText(text)
        self.db_status.setStyleSheet(f"""
            QLabel {{
    ...
    def update_db_mode_status(self):
        mode = self.db_mode_selector.currentText()
    
        if mode == "SQLite":
            self.db_status.setText("SQLite (default)")
            self.db_status.setStyleSheet("""
                QLabel {
                    background-color: #f39c12;
                    color: white;
            ...
            self.db_status.setText(f"{mode} Not Connected")
            self.db_status.setStyleSheet("""
                QLabel {
                    background-color: #95a5a6;
                    color: white;
    ...
    def load_csv(self):
        ...
        if self.table_selector.count() > 1:
    ...
    def get_keywords(self):
        mode = self.db_mode_selector.currentText()
        common = set(self.keyword_data.get("COMMON", []) or [])
        specific = set(self.keyword_data.get(mode, []) or [])
        return sorted(common.union(specific))
    ...
    def connect_postgres_db(self):
        ...
        conn = self.db_manager.connect_postgres(host, db, user, password)    
        if conn:
            self.query_status.setText("PostgreSQL connected successfully")
            self.update_db_status("PostgreSQL Connected")
            self.db_status.setStyleSheet("""
                QLabel {
                    background-color: #27ae60;
                    color: white;
            ...
            self.query_status.setText("PostgreSQL connection failed")
            
    def connect_postgres_ui(self):
        host = self.pg_host_input.text().strip()
        db = self.pg_db_input.text().strip()
        user = self.pg_user_input.text().strip()
        password = self.pg_pass_input.text().strip()
        if not host or not db or not user:
            self.query_status.setText("Fill Host, DB, User")
            return    
        mode = self.db_mode_selector.currentText()
        conn = None
        if mode == "PostgreSQL":
            conn = self.db_manager.connect_postgres(
            ...
            conn = self.db_manager.connect_oracle(
                host, db, user, password
            )
        else:
            self.query_status.setText(
                f"{mode} does not require external connection"
            )
            return    
        if conn:
            self.query_status.setText(
                f"{mode} connected successfully"
            )
            self.db_status.setText(f"{mode} Connected")
            self.db_status.setStyleSheet("""
                QLabel {
                    background-color: #27ae60;
                    color: white;
            ...
            self.query_status.setText(
                f"{mode} connection failed"
            )
            self.db_status.setText(f"{mode} Failed")
            self.db_status.setStyleSheet("""
                QLabel {
    ...
    def disconnect_db(self):
        try:
            for db in ["PostgreSQL", "MySQL", "Oracle"]:
                conn = self.db_manager.get_connection(db)
                if conn:
                    conn.close()
                    self.db_manager.connections.pop(db, None)
        except Exception:
            pass
        self.db_status.setText("SQLite (default)")
        self.db_status.setStyleSheet("""
    ...
    def save_pg_connection(self):
        name = self.pg_name_input.text().strip()
        if not name:
            self.query_status.setText("Host is required to save connection")
            return    
        self.saved_pg_connections[name] = {
            "host": self.pg_host_input.text().strip(),
            "db": self.pg_db_input.text().strip(),
            "user": self.pg_user_input.text().strip(),
            "password": self.pg_pass_input.text().strip(),
        ...
        if self.pg_saved_dropdown.findText(name) == -1:
            self.pg_saved_dropdown.addItem(name)
        with open(self.pg_conn_file, "w") as f:
            json.dump(self.saved_pg_connections, f, indent=2)
        self.query_status.setText(f"Saved connection: {name}")
        
    def delete_pg_connection(self):
        name = self.pg_saved_dropdown.currentText()    
        if name == "Select Saved Connection" or not name:
            self.query_status.setText("No connection selected to delete")
            return
        if name in self.saved_pg_connections:
            del self.saved_pg_connections[name]    
            try:
                with open(self.pg_conn_file, "w") as f:
                    json.dump(self.saved_pg_connections, f, indent=2)
            except Exception as e:
                self.query_status.setText(f"Delete failed: {str(e)}")
                return
            index = self.pg_saved_dropdown.currentIndex()
            self.pg_saved_dropdown.removeItem(index)            
            if self.pg_saved_dropdown.count() == 0:
                self.pg_saved_dropdown.addItem("Select Saved Connection")   
            self.query_status.setText(f"Deleted connection: {name}")
        
    def load_saved_pg_connection(self):
        name = self.pg_saved_dropdown.currentText()    
        if name == "Select Saved Connection":
            return
        data = self.saved_pg_connections.get(name)
        if not data:
            return    
        self.pg_host_input.setText(data["host"])
        self.pg_db_input.setText(data["db"])
        self.pg_user_input.setText(data["user"])
        self.pg_pass_input.setText(data["password"])
    ...
    def run_query(self):
        ...
        try:
            ...
            if mode == "PostgreSQL":
                conn = self.db_manager.get_connection("PostgreSQL")
                if conn is None:
                    conn = self.db_manager.reconnect_postgres()
            if conn is None:
                self.query_status.setText("Database not connected")
                return
            cursor = conn.cursor()
    ...
    def load_pg_connections(self):
        if os.path.exists(self.pg_conn_file):
            try:
                with open(self.pg_conn_file, "r") as f:
                    self.saved_pg_connections = json.load(f)
            except:
                self.saved_pg_connections = {}
        else:
            self.saved_pg_connections = {}
        for name in self.saved_pg_connections.keys():
            if self.pg_saved_dropdown.findText(name) == -1:
                self.pg_saved_dropdown.addItem(name)
            
    def save_pg_connections_to_file(self):
        try:
            with open(self.pg_conn_file, "w") as f:
                json.dump(self.saved_pg_connections, f, indent=2)
        except Exception as e:
            self.query_status.setText(f"Save failed: {str(e)}")        
    
    def auto_reconnect_last_pg(self):
        if not hasattr(self, "last_pg_connection"):
            return
        data = self.last_pg_connection
        if not data:
            return    
        conn = self.db_manager.connect_postgres(
            data["host"],
        ...
        if conn:
            self.query_status.setText("Auto-connected to last PostgreSQL session")
    ...
    def show_help(self):
        dialog = QDialog(self)
        dialog.setWindowTitle("SQL Keywords Help")
        dialog.resize(600, 500)
        layout = QVBoxLayout(dialog)    
        text = QTextEdit()
        text.setReadOnly(True)
        content = ""
        for db, keywords in (self.keyword_data or {}).items():
            if not isinstance(keywords, (list, tuple)):
                continue
            content += f"{db}:\n"
            content += ", ".join(sorted([k for k in keywords if isinstance(k, str)]))
            content += "\n\n"
    ...
    def auto_fill_conn_name(self):
        if not self.pg_name_input.text().strip():
            host = self.pg_host_input.text().strip()
            if host:
                self.pg_name_input.setText(f"{host}_postgres")
