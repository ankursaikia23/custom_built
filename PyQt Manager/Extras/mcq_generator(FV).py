...
from PyQt5.QtWidgets import (
    ...
    QHBoxLayout, QComboBox, QMessageBox, QFileDialog, QTableWidget, QTableWidgetItem,
...
class QuizMainWindow(QMainWindow):
    ...
    def start_quiz(self):
        global df_all, score, attempted, correct_count, skipped_count, review_log, marked_errors, questions_master
    ...
    def answer(self, btn, row):
        ...
        if btn.text() == str(row[answer_col]):
            ...
            print("Saving correct question to:", CORRECT_FILE)
        ...
        self.feedback.setText(self.feedback.text() + f"<br>Attempted: {attempted} | Correct: {correct_count} | Skipped: {skipped_count} | Score: {score}")
        self.show_controls()
    ...
    def mark(self):
        ...
        self.feedback.setText(self.feedback.text() + "<br>⚠️ Marked for review.")
        global skipped_count, attempted
        skipped_count += 1
        attempted += 1
        if self.index+1 < len(self.questions):
    ...
    def show_controls(self):
        ...
        if self.index+1 < len(self.questions):
            ...
            end_now = QPushButton("Finish")
            end_now.setStyleSheet("background-color:#337ab7;color:white;font-size:25px;")
            end_now.setMinimumHeight(56)
            self.ctrl_layout.addWidget(end_now)
            end_now.clicked.connect(self.end)
    ...
    def end(self):
        ...
        if review_log:
            dfv = pd.DataFrame(review_log, columns=["Status", "Question", "Answer"])
            self.review.setRowCount(len(dfv))
            self.review.setColumnCount(3)
            ...
            self.review.horizontalHeader().setSectionResizeMode(QHeaderView.Stretch)
        if marked_errors:
            records = []
