...
def extract_questions_from_text(text):
    ...
    while i < len(lines):
        ...
        if re.match(r"ques(tion)?\s*:", line, re.IGNORECASE):
            q_first = line.split(":", 1)[1].strip()
            question_text, pos = collect_block(
                i + 1,
                [lbl + ")" for lbl in option_labels] + [lbl + "." for lbl in option_labels] +
                ["Answer", "ANSWER", "Ans", "Correct"]
            )
            if q_first:
            ...
            for idx, label in enumerate(option_labels):
                if pos < len(lines):
                    line_option = lines[pos].strip()
                    if re.match(f"^{label}[.)]", line_option):
                        first = line_option[2:].strip()
                        next_labels = []
                        if idx < 4:
                            next_labels.extend([option_labels[idx + 1] + ")", option_labels[idx + 1] + "."])
            ...
            while pos < len(lines):
                ...
                if al_low.startswith(("answer", "ans", "correct")):
                    ...
                    m = re.search(r"[Oo]ption\s*([A-Ea-e])", after)
                    if not m:
...
class PDFExtractorThread(QThread):
    ...
    def run(self):
        ...
        for pdf_file in pdf_files:
            ...
            mod_pdf_path = os.path.join(self.output_folder, f"mod_{pdf_file}")
            save_modified_pdf(mod_text, mod_pdf_path)
            if rows:
