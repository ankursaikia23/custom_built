...
class CSVEditor(QWidget):
    ...
    def __init__(self):
        ...
        self.autosave_timer.timeout.connect(self.auto_save_csv)
    ...
    def save_csv(self):
        ...
        if not hasattr(self, "current_csv_path") or not self.current_csv_path:
            self.csv_status_label.setText("No file path found to save.")
            return
        ...
        try:
            ...
            self.csv_status_label.setText(f"Error saving file: {e}")
    ...
    def auto_save_csv(self):
        ...
        try:
            ...
            self.update_dataframe_from_table()
            self.df.to_csv(self.current_csv_path, index=False)
            self.csv_status_label.setText(f"💾 Auto-saved at {pd.Timestamp.now().strftime('%H:%M:%S')}")
        except Exception as e:
            self.csv_status_label.setText(f"Auto-save failed: {e}")
